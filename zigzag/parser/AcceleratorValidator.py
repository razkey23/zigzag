...
class AcceleratorValidator:
    ...
    def validate(self) -> bool:
        ...
        for mem_name in self.data["memories"]:
            print("Now validating ", mem_name)

            self.validate_single_memory(mem_name)
            if not self.is_valid:
                print(mem_name, " is invalid")
    ...
    def validate_single_memory(self, mem_name: str) -> None:
        ...
        if nb_ports != nb_operands:
            print("Invalid :", f"Number of memory ports ({nb_ports}) does not equal number of operands ({nb_operands}) for {mem_name}" )
            self.invalidate(
        ...
        for served_dimension in mem_data["served_dimensions"]:
            print(f"Invalid served dimension {served_dimension} in memory {mem_name}")
        ...
        if len(w_ports) != mem_data["w_port"]:
            print(f"Number of given write ports ({mem_data['w_port']}) does not equal number of allocated write ports ")
            self.invalidate(
                f"Number of given write ports ({mem_data['w_port']}) does not equal number of allocated write ports "
                f"({len(w_ports)}) for {mem_name}"
            )
        if len(rw_ports) != mem_data["rw_port"]:
            print(f"Number of given read/write ports ({mem_data['rw_port']}) does not equal number of allocated ")
            self.invalidate(
