...
from zigzag.hardware.architecture.macro.Dac import Dac
from zigzag.hardware.architecture.macro.Adc import Adc
from zigzag.hardware.architecture.macro.Adder import Adder
from zigzag.hardware.architecture.macro.Cell import Cell
from zigzag.hardware.architecture.macro.Reg import Reg

class _ImcUnit(OperationalArrayABC):
    TECH_PARAM_28NM = {
    ...
    def __init__(
        ...
        self.tech_param = _ImcUnit.TECH_PARAM_28NM
        ...
        # Changes -> decoupled 
        self.adc = Adc(self.tech_param,self.adc_resolution)
        self.cell = Cell(self.tech_param)
        self.dac = Dac(self.tech_param)
        self.adder = Adder(self.tech_param)
        self.reg = Reg(self.tech_param)
        ...
        if bl_dim in spatial_mapping:
            ...
            if not weight_ir_loop_on_wl_dim:
                ...
                mapped_rows_total_per_macro, mapped_rows_for_adder_per_macro = (
                    _ImcUnit.calculate_mapped_rows_when_diagonal_mapping_found(
                        layer,
                        layer_const_operand,
                        layer_act_operand,
    ...
    def get_regular_adder_trees_energy(
        ...
        if mapped_inputs >= 1:
            ...
            single_adder_tree_energy = (
                fully_activated_number_of_1b_adder * self.adder.get_1b_adder_energy()
                + half_activated_number_of_1b_adder * self.adder.get_1b_adder_energy_half_activated()
            )
        else:
