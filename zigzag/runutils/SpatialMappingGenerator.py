...
class SpatialMappingGenerator():
    ...
    def run(self):
        ...
        for i, generated_mapping in enumerate(generated_mappings):
            ...
            # Since we do not run callable stages, we store the mapping and any essential info directly
            results.append({
                'layer_name': self.layer.name,
                'mapping_index': i + 1,
                'total_mappings': nb_generated_mappings,
                'spatial_mapping': generated_mapping,
                'modified_accelerator': accelerator_under_test  # Optional, include if relevant
            })
